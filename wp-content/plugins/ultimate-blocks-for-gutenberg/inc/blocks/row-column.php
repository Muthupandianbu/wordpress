...
class Easy_Blocks_Gutenberg_Row_Columns{
    ...
    public function __construct(){
        if(!is_admin()){            
            add_action( 'init', array( $this, 'egb_init' ) );
            add_action( 'wp_enqueue_scripts', array( $this, 'egb_frontend_inline_css' ), 20 );
        }
    }


    public function egb_frontend_inline_css(){
    	include_once( UGB_PLUGIN_DIR . '/ultimate-gutenberg-blocks.php' );
		$egb = new Ultimate_Gutenberg();
		...
		$blocks = $egb::egb_parse_blocks( $post->post_content );
		// print_r( $blocks );
		...
		foreach ( $blocks as $indexkey => $block ) {
			if ( ! is_object( $block ) && is_array( $block ) && isset( $block['blockName'] ) ) {
				if ( 'easy-blocks/row-column' === $block['blockName'] ) {
				...
				if ( 'easy-blocks/column' === $block['blockName'] ) {
					if ( isset( $block['attrs'] ) && is_array( $block['attrs'] ) ) {
						$blockattr = $block['attrs'];
	...
	public function egb_init(){
		...
		register_block_type( 'easy-blocks/row-column', array(
			'render_callback' => array( $this, 'egb_row_layout_css_render' ),
		) );
